...
export const LABOR_SEASONAL_COST: Record<LaborType, { min: number; max: number }> = {
  ...
  [LaborType.IndenturedBlack]:   { min: 0,   max: 0   },  // provisions only during contract term; acquisition covers passage fee
  [LaborType.IndenturedWhite]:   { min: 0,   max: 0   },  // provisions only during contract term; acquisition covers passage fee
...
// ---------------------------------------------------------------------------
// INDENTURE — freedom dues on contract expiry
// ---------------------------------------------------------------------------

/**
 * Freedom dues paid to an indentured servant at end of contract.
 * Historically: corn + clothing + tools. Land grants were uncommon.
 *
 * Player is offered two options at expiry:
 *   A) Pay cash + corn (standard freedom dues)
 *   B) Pay cash buyout (higher cost; no corn; historically a planter
 *      buying out the servant's entitlement to goods in cash)
 *
 * Source: Virginia statute 1705 — 10 bushels corn, 30s cash, 20s musket.
 * Scaled to game units: corn from stores, cash equivalent.
 */
export const FREEDOM_DUES_CORN_UNITS  = 5    // units drawn from cornOnHand
export const FREEDOM_DUES_CASH        = 30   // cash paid alongside corn
export const FREEDOM_DUES_BUYOUT_CASH = 80   // cash-only buyout alternative

// ---------------------------------------------------------------------------
// CROP WORK COMPOST YIELD
// ---------------------------------------------------------------------------

/**
 * Compost material generated by crop work activities, as a fraction of
 * the timber-clearing baseline (CLEARED_MATERIAL_YIELD[Forest] = 2 units).
 *
 * Rates reflect the relative biomass available from each activity:
 *   - Timber clearing: baseline (2–3 units; large woody material)
 *   - Crop clearing:   60% — removing standing stalks/roots has real bulk
 *   - Harvesting:      30% — plant residue left after harvest
 *   - Tending:         15% — weeding and pruning; small volume
 *
 * Values are fractional chances of generating 1 unit per worker per season.
 * A 0.30 rate means a harvesting worker has a 30% chance of contributing
 * 1 unit of compost material that season.
 */
export const CROP_COMPOST_YIELD_CHANCE = {
  clearing: 0.60,   // ClearLand task on a tile with a crop
  harvesting: 0.30, // HarvestCrop task
  tending: 0.15,    // TendCrop task
}
